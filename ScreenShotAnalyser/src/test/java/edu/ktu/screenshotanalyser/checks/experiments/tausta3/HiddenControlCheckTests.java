...
public class HiddenControlCheckTests extends HiddenControlCheck
{		
		...
		@ParameterizedTest
		@MethodSource
		public void testColorDifference(Color color2, double expected) throws IOException
		{
			Color color1 = new Color(30, 50, 80);
			double actual = ColorDifference(color1, color2);
	
			Assert.assertEquals(expected, actual, 0.1);
		}
		private static Stream<Arguments> testColorDifference()
		{
			List<Arguments> testCases = new ArrayList<>();
			Color color = new Color(25, 40, 158);
			testCases.add(Arguments.of(color, 57.1966));
			
			color = new Color(74, 138, 94);
			testCases.add(Arguments.of(color, 59.8136));
			
			color = new Color(213, 5, 12);
			testCases.add(Arguments.of(color, 104.0813));
			return testCases.stream();
		}
		...
		private static Stream<Arguments> testRuleCheck()
		{
			...
			when(state.getImageFile()).thenReturn(new File("C:\\gui\\_test_\\HiddenControlTest1.png"));
			...
			numFailures = 1;
			when(state.getActualControls()).thenReturn(controls);
			when(state.getImageFile()).thenReturn(new File("C:\\gui\\_test_\\HiddenControlTest2.png"));
			testCases.add(Arguments.of(state, failuresFound, numFailures));
			...
			numFailures = 2;
			when(state.getActualControls()).thenReturn(controls);
			when(state.getImageFile()).thenReturn(new File("C:\\gui\\_test_\\HiddenControlTest3.png"));
			testCases.add(Arguments.of(state, failuresFound, numFailures));
